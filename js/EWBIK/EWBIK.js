...
export class EWBIK extends Saveable {
    ...
    setDefaultIterations(iterations) {
        this.defaultIterations = parseInt(iterations);
    ...
    setDefaultStabilizingPassCount(passCount) {
        this.defaultStabilizingPassCount = parseInt(passCount);
    }
    ...
    noOp(fromBone = null, iterations = this.getDefaultIterations(), callbacks) {
        ...
        this.shadowSkel.noOp(fromBone, iterations, (wb) => this.solveCompleted(wb), callbacks);
     ...
     * @param {CallbacksSequence} debug_callbacks to snoop on solver state
     **/
    async __solve(bone, literal, iterations = this.getDefaultIterations(), stabilizingPasses = this.getDefaultStabilizingPassCount(), onComplete = (wb) => this.solveCompleted(wb), debug_callbacks) {
        if (this.dirtyShadowSkel)
            this._regenerateShadowSkeleton();
        if (this.dirtyRate)
            this._updateShadowSkelRateInfo(iterations);
     ...
     */
    async doSinglePullbackStep(bone, stopOn, iterations = this.getDefaultIterations(), stabilizingPasses = this.getDefaultStabilizingPassCount(), onComplete = (wb) => this.solveCompleted(wb), callbacks) {
        let literal = stopOn != null;
     ...
     * @param {CallbacksSequence} callbacks to snoop on solver state
     */
    async _doSinglePullbackStep(bone = null, literal, iterations = this.getDefaultIterations(), stabilizationPasses = this.getDefaultStabilizingPassCount(), onComplete = (wb) => this.solveCompleted(wb), callbacks) {
        ...
        callbacks?.__initStep((callme, wb) => { his.stepWiseUpdateResult(callme, wb); this.solveCompleted(wb) });
    ...
    /**
     * The solver can notify you every time it has completed a solve call. The provided callback is triggered once per bone per solve call (not one per iteration)
     * @param {Function} callback 
     */
    registerPostSolveCallback(callback) {
        if (this._default_post_solve_callbacks.indexOf(callback) == -1) {
            this._default_post_solve_callbacks.push(callback);
        }
    }

    removePostSolveCallback(callback) {
        let cbidx = this._default_post_solve_callbacks.indexOf(callback);
        if(cbidx > -1) {
            this._default_post_solve_callbacks.splice(cbidx, 1);
        }
    }

    stepWiseUpdateResult(callMe, wb) {
        this.solveCompleted(wb);
        callMe(wb);
    }

    solveCompleted(wb) {
        this.alignBoneToSolverResult(wb);
        for (let cb of this._default_post_solve_callbacks) {
            cb(wb);
        }
    }
     ...
     */
    inferOrientations(fromBone = this.rootBone, mode, override = false, depth = Infinity) {
     ...
     * lets you manualy specify the direction the physical bone should point in relative to the bone frame
    ...
    setInternalOrientationFor(forBone, pointTo) {
        if (forBone.getConstraint() != null) {
            throw new Error("This function cannot be used on bones that have already had constraints defined. It's dangerous and complicated, but the short version is you should not add constraints until you're sure you know which way your bones are supposed to be pointing.")
        }
    ...
    */
    ...
    _debug_bone(solveFrom, iterations = this.defaultIterations, callbacks = null) {
        ...
        callbacks?.__initStep((callme, wb) => this.stepWiseUpdateResult(callme, wb));
        this.shadowSkel.debug_solve(iterations, this.getDefaultStabilizingPassCount(), solveFrom, (wb) => this.solveCompleted(wb), callbacks);
    }
...
let betterbones = {
    ...
    */
    getInferredHeight(mode = 'statistical', minHeight = 0.001) {
        ...
        let thisWPos = this.getWorldPosition(this.position.clone());
        let thisWVec = pool.any_Vec3().readFromTHREE(thisWPos);
        let childWPos = this.position.clone();
        let sumheight = 0;
        let count = 0;
        let minChild = 999;
        let maxChild = -1;
        ...
        for (let cb of this.childBones()) {
            count++;
            cb.getWorldPosition(childWPos);
            let childvec = pool.any_Vec3(childWPos.x, childWPos.y, childWPos.z).sub(thisWVec);
            let childMagSq = childvec.magSq();
            let childMag = Math.sqrt(childMagSq);
            sum_sqheight += childMagSq; //helps weigh in favor of further bones, because a lot of rigs do weird annoying things with twist bones.
            sumheight += childMag;
...
Object.defineProperty(THREE.Bone.prototype, '_height', { //for notifying any IKPins of inferred height changes
    ...
    set: function (newValue) {
        ...
        if (this.getIKPin() != null) this.getIKPin().setPSTPriorities();
    },
    enumerable: true,
    configurable: true
});

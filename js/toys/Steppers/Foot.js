...
import {ActiveBalance, ProjectionsObj, Proposal } from "./ActiveBalance.js";
...
export class Foot {
    ...
    /**
     * 
     * @param {Foot} otherFoot reference to the other foot object on the balancer
     * @param {ActiveBalance} hips reference to the balancing class managing this foot
     * @param {Vec3} attachpoint point where the leg this foot is on attaches to the hips
     * @param {Number} major_maxStrideAngle the halfangle for the major axis of the elliptical cone emerging from attachpoint which defines a valid region of concern for this foot. The foot will not attempt to plant itself beyond this region.
     * @param {Number} minor_maxStringAngle the halfangle for the minor axis the elliptical cone emerging from attachpoint which defines a valid region of concern for this foot. The foot will not attempt to plant itself beyond this region.
     * @param {*} footPosition 
     * @param {*} isPlanted 
     */
    constructor(otherFoot, hips, attachpoint, major_maxStrideAngle, minor_maxStringAngle, footPosition, isPlanted=true) {
        ...
        this.major_maxStrideAngle = major_maxStrideAngle;
        this.minor_maxStrideAngle = minor_maxStringAngle;
    ...
    /**
     * 
     * @param {Vec3} pos expected foot position relative to attachpoint
     * @returns true if the position is within the valid reach region, false otherwise
     */
    isPointInsideEllipticalCone(pos) {
        if (y >= 0) {
            return false;
        }
        const tanThetaZ = Math.tan(this.major_maxStrideAngle);
        const tanThetaX = Math.tan(this.minor_maxStrideAngle);
        
        const zRatioSquared = (pos.z / (-pos.y * tanThetaZ)) ** 2;
        const xRatioSquared = (pos.x / (-pos.y * tanThetaX)) ** 2;
        
        return (zRatioSquared + xRatioSquared) <= 1;
    }
